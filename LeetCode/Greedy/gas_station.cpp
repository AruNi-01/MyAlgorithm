...
贪心算法：
    ...
    情况二：如果 gas[i] - cost[i] 的总和 totalSum 不为负数，那么肯定可以绕一圈：
            如果在区间 [i, j] 内的总和curSum为负数，说明 [i, j] 区间都不能作为起始位置，
            起始位置从 j + 1 算起，再从0计算curSum。
            如果再出现负数，就再更新起始位置为 j + 1，因为此情况已经确定了肯定可以绕一圈的，
            所以j之前出现了多少负数，j后面就会出现多少正数。
    如图：https://img-blog.csdnimg.cn/20201213162821958.png
*/
class Solution {
public:
